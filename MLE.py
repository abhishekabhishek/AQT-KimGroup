...
#         L += (q-p[i])**2/q
    ...
    for nq in range(Nq):
        rho_reduced = np.zeros((Na*len(rho), int(len(rho[0])/2),
                                int(len(rho[0])/2)), dtype=complex)
        for i in range(len(rho)):
            for na in range(Na):
                rho_reduced[Na*i+na] = rho[i, 0::2, 0::2]*M[na, 0, 0] \
                    + rho[i, 1::2, 0::2]*M[na, 0, 1]+rho[i, 0::2, 1::2] \
                    * M[na, 1, 0]+rho[i, 1::2, 1::2]*M[na, 1, 1]
        rho = rho_reduced
...
def MLE_DM(ptab, Nq, povm):
    ...
    t = time.time()
    res = minimize(MLELoss, x0=x0, args=(ptab, Nq, povm),
